...
export default function Referee({id}: Props) {
    ...
    useEffect(() => {
        const fetchBoard = () => {
            axios.get(`http://34.233.174.30:5000/show_boards/${boardID}`)
                .then(response => {
                    set_vars(response);
                })
    ...
    async function playMove(playedPiece: Piece, destination: Position): Promise<boolean> {
        try {
            const response = await axios.post(`http://34.233.174.30:5000/play_move/${boardID}`, {'id': playedPiece.id, 'x': destination.x, 'y': destination.y});
            set_vars(response);
            playedMoveIsValid = response.data["result"];
            if (response.data["total_turns"] % 2 === 0 && playedMoveIsValid) {
                const response_ = await axios.post(`http://34.233.174.30:5000/ai_move/${boardID}`);
                set_vars(response_);
            }
        } catch (error) {
    ...
    function promotePawn(pieceType: PieceType) {
        ...
        axios.post(`http://34.233.174.30:5000/promote_pawn/${boardID}`, {'id': promotionPawn.id, 'piece_type': pieceType})
            .then(response => {
                set_vars(response);
            })
            .catch(error => console.error('Failed to fetch board:', error));
    ...
    function restartGame() {
        checkmateModalRef.current?.classList.add("hidden");
        stalemateModalRef.current?.classList.add("hidden");
        axios.post(`http://34.233.174.30:5000/reset_board/${boardID}`)
            .then(response => {
                set_vars(response);
            })
            .catch(error => console.error('Failed to fetch board:', error));
